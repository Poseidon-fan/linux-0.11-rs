...
impl IrqSaveGuard {
    ...
    #[inline]
    pub fn enter() -> Self {
        let (_, depth) = task::current_irq_state();
        let outer_if_enabled = (depth == 0).then(save_and_disable);
        task::set_current_irq_state(outer_if_enabled, Some(depth + 1));
        Self
    }
}
...
impl Drop for IrqSaveGuard {
    fn drop(&mut self) {
        let (saved_if, depth) = task::current_irq_state();
        assert!(depth > 0, "IrqSaveGuard depth underflow");

        let next = depth - 1;
        if next == 0 {
            task::set_current_irq_state(Some(false), Some(0));
            restore(saved_if);
        } else {
            task::set_current_irq_state(None, Some(next));
        }
    }
}
